...
The confirmatory full-cohort run (`configs/full.yaml`) has since executed and passed every gate — environment validation, data preparation, structural validation, benchmark (19,500/19,500 conditions, 0 failed), result-integrity audit, aggregation, and figure generation — on 65 structurally validated participants. See [`docs/full_run_acceptance.md`](docs/full_run_acceptance.md) for the closure record. This closes execution only; scientific interpretation of the resulting numbers is a separate, not-yet-performed task, and no performance claim is made in this README.
